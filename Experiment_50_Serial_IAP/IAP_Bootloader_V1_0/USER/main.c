...
int main(void)
{
	...
	while (1)
	{
		...
		if (key != KEY_S_PRES && first_run_flag==1)	//上电后检测到 立即检测到启动开关是按下状态，开始执行固件更新
		{
			printf("开始执行FLASH用户代码!!\r\n");
			if (((*(vu32*)(FLASH_APP1_ADDR + 4)) & 0xFF000000) == 0x08000000)//判断是否为0X08XXXXXX.
			{
		...
		if (key == KEY_S_PRES && first_run_flag == 0)	//上电后检测到 立即检测到启动开关是按下状态，开始执行固件更新
		{
			if (applenth)
			{
				printf("开始更新固件...\r\n");
